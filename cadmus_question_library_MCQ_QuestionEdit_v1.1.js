...
(async () => {
  // ─── CONFIG ────────────────────────────────────────────────────────────────
  const OPTIONS = {
    points:  1,     // number  – points per question
    shuffle: true,  // boolean – shuffle answer choices
  };
  // ───────────────────────────────────────────────────────────────────────────
  ...
  const rows    = allRows.filter(r => r.original.questionType === 'MCQ');
  const skipped = allRows.length - rows.length;
  if (skipped > 0) console.warn(`⚠ Skipped ${skipped} non-MCQ question(s)`);
  if (rows.length === 0) { alert('No MCQ questions selected'); return; }
  console.log(`Processing ${rows.length} MCQ question(s)…`);
    ...
    const input = {
      ...
      attributes: {
        ...
        fields: [{
          ...
          choiceInteraction: {
            choices:    inter.choices.map(c => ({ identifier: c.identifier, content: c.content })),
            maxChoices: inter.maxChoices,
          },
        }],
      },
    };
